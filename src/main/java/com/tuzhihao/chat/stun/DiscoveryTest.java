...
package com.tuzhihao.chat.stun;


import com.tuzhihao.chat.stun.attribute.*;
import com.tuzhihao.chat.stun.header.MessageHeader;
import com.tuzhihao.chat.stun.header.MessageHeaderParsingException;
import com.tuzhihao.chat.stun.util.UtilityException;
    ...
    private boolean test1() throws UtilityException, SocketException, UnknownHostException, IOException,
        ...
        while (true) {
            try {
                ...
                if ((ma == null) || (ca == null)) {
                    di.setError(700,
                            "The server is sending an incomplete response (Mapped Address and Changed Address message"
                                    + " attributes are missing). The client should not retry.");
                    ...
                    if ((ma.getPort() == socketTest1.getLocalPort()) && (ma.getAddress()
                            .getInetAddress()
        ...
        while (true) {
            try {
                ...
                if (!nodeNatted) {
                    di.setOpenAccess();
                    LOGGER.debug(
                            "Node has open access to the Internet (or, at least the node is behind a full-cone NAT "
                                    + "without translation).");
                } else {
                    di.setFullCone();
    ...
    private boolean test1Redo() throws UtilityException, SocketException, UnknownHostException, IOException,
        ...
        while (true) {
            ...
            try {
                ...
                if (ma2 == null) {
                    di.setError(700,
                            "The server is sending an incomplete response (Mapped Address message attribute is "
                                    + "missing). The client should not retry.");
                    LOGGER.debug("Response does not contain a Mapped Address message attribute.");
                    return false;
                } else {
                    if ((ma.getPort() != ma2.getPort()) || (!(ma.getAddress()
                            .getInetAddress()
                ...
                if (timeSinceFirstTransmission < 7900) {
                    ...
                    LOGGER.debug(
                            "Test 1 redo with changed address: Socket timeout while receiving the response.  Maximum "
                                    + "retry limit exceed. Give up.");
                    return false;
                }
            }
        }
